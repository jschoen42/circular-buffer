...
class CircularBufferMinimalSizeError(Exception):
    def __init__(self) -> None:
        super().__init__("Circular Buffer minimal size is 1")

class CircularBufferEmptyError(Exception):
    def __init__(self) -> None:
        super().__init__("Circular Buffer ist empty")

class CircularBufferFullError(Exception):
...
class CircularBuffer:
    def __init__(self, size: int=100) -> None:
        ...
        if size < 1:
            raise CircularBufferMinimalSizeError
    ...
    def write(self, value: Any) -> None:
        ptr = (self.head + 1) % self.size
        if ptr == self.tail:
            raise CircularBufferFullError( self.size-1 )
    ...
    def read(self) -> Any:
        if self.tail == self.head:
            raise CircularBufferEmptyError
